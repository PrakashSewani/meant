...
## v0.1 — "One loop, done well" *(internal dogfood)*
...
- **Recipes:** three universal ones — *Say it better*, *Make it shorter*, *Fix the tone*.
...
- **Measured:** the inference golden corpus lands *with* the register engine, not after it —
...
## v1.0 — "Everywhere, one loop" *(public beta)*
...
- Free trials need server-side metering, quotas, and abuse controls; metering must be **visible to
  the user**, and it is the *only* new outbound destination this adds.
...
| A *required* hosted tier | BYOK/local stay the free, private path forever. An optional hosted tier is planned only after traction — see "Beyond v2". |
...
| "Another writing extension" perception | High | High | Lead with the *register* story and the no-chat-box, no-prompt promise; demo the 3-second loop. |
