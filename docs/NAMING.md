...
Scored 1–5. We care most about *distinctiveness* and *meaning fit*, because those are what make
...
| --- | :--: | :--: | :--: | :--: | :--: | --- |
| **Meant** *(current codename)* | 5 | 2 | 5 | 4 | High | Tagline writes itself ("what you mean"). But it's an everyday word: terrible search, crowded mark, easy to mishear. |
| **Sayable** *(recommended)* | 5 | 5 | 4 | 5 | Low | "Make what you mean sayable." Coined, ownable, warm, works as noun and verb-adjacent. Minor: reads like a utility (`-able`). |
...
Rationale: it names the *outcome* the user wants ("make this sayable") in one coined word that
is easy to own, easy to say, and has no baggage. It works in a sentence — *"just make it
sayable"* — which is the test most names fail. It also extends cleanly: the action is "sayable
it," the surface is the "Sayable bar," and the tagline is the existing one.

**Tagline:** *Say what you mean.* (Lineage: the original "turn what you mean into what you say"
...
- **Category we claim:** *the register layer for writing* (not "AI writing assistant").
