...
| `reasoning_model` | `"provider/model"` | Deep tier — long docs, nuanced register. *(Sayable extension; ignored by OpenCode.)* |
...
| OpenAI | openai-compatible | `https://api.openai.com/v1` | Blocked from *pages* by design; works from the worker. |
...
- With a passphrase set, the vault starts locked at browser launch, unlocks into
  `chrome.storage.session`, and fails closed: a locked transform offers *Unlock to transform*
